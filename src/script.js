document.addEventListener('DOMContentLoaded', () => {
    ...
    const initDummyData = () => {
        const dummyUsers = [
            { id: 1, name: 'Admin', username: 'admin', contact: 'admin@example.com', password: 'Password123!' },
            { id: 2, name: 'Alice', username: 'alice', contact: 'alice@example.com', password: 'alice123' },
            { id: 3, name: 'Bob', username: 'bob', contact: 'bob@example.com', password: 'bob456' },
            { id: 4, name: 'Charlie', username: 'charlie', contact: 'charlie@example.com', password: 'charlie789' }
        ];

        const dummyQuestions = [
            { id: 101, title: 'How to manage state in a large React application?', description: '<p>I\'m building a complex app with many components that need to share state. Prop drilling is becoming a nightmare. What are the best practices for state management? I\'ve heard of Redux and Context API, but I\'m not sure which one to choose.</p>', tags: ['react', 'state-management', 'javascript'], author: 'alice', votes: 15, createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: 202 },
            { id: 102, title: 'What is the difference between `let`, `const`, and `var` in JavaScript?', description: '<p>I\'m new to modern JavaScript and I see these three ways to declare variables. When should I use each one? What are the scoping rules for them? An example would be very helpful!</p>', tags: ['javascript', 'es6', 'variables'], author: 'bob', votes: 22, createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: null },
            { id: 103, title: 'How can I center a div both horizontally and vertically with CSS?', description: '<p>This seems like it should be simple, but I always struggle with it. I\'ve tried using margins, positioning, and other tricks, but they don\'t always work. What is the most reliable, modern way to perfectly center an element using CSS Flexbox or Grid?</p>', tags: ['css', 'flexbox', 'layout'], author: 'charlie', votes: 30, createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: 201 },
            { id: 104, title: 'Best way to fetch data in a React component?', description: '<p>What is the recommended way to fetch data from an API when a component mounts? Should I use the `useEffect` hook? Are there any libraries that make this easier, like SWR or React Query?</p>', tags: ['react', 'api', 'hooks'], author: 'alice', votes: 8, createdAt: new Date(Date.now() - 10 * 60 * 1000).toISOString(), acceptedAnswerId: null },
            { id: 105, title: 'What are closures in JavaScript?', description: '<p>I keep hearing about closures but I don\'t fully understand what they are or why they are useful. Can someone explain it in simple terms?</p>', tags: ['javascript', 'closures', 'functions'], author: 'bob', votes: 18, createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: null },
            { id: 106, title: 'How to use async/await with Array.prototype.map()?', description: '<p>I am trying to use an async function inside a `.map()` to process an array of items, but it seems to return an array of Promises instead of the resolved values. How can I correctly use `async/await` with `map`?</p>', tags: ['javascript', 'async-await', 'promises'], author: 'charlie', votes: 11, createdAt: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: null },
            { id: 107, title: 'What is the purpose of a .gitignore file?', description: '<p>I see a `.gitignore` file in almost every project repository. What does it do and what kind of files should I include in it?</p>', tags: ['git', 'version-control'], author: 'alice', votes: 25, createdAt: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: null },
            { id: 108, title: 'CSS Grid vs. Flexbox: Which one should I use?', description: '<p>When should I use CSS Grid and when should I use Flexbox? What are the key differences and strengths of each layout model?</p>', tags: ['css', 'grid', 'flexbox', 'layout'], author: 'bob', votes: 19, createdAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: null },
            { id: 109, title: 'How to properly handle errors in an Express.js application?', description: '<p>What is the standard way to handle errors in an Express app? Should I use middleware? How do I handle both synchronous and asynchronous errors?</p>', tags: ['nodejs', 'express', 'error-handling'], author: 'charlie', votes: 14, createdAt: new Date(Date.now() - 12 * 24 * 60 * 60 * 1000).toISOString(), acceptedAnswerId: null },
        ];

        const dummyAnswers = [
            { id: 201, questionId: 103, author: 'alice', votes: 18, content: '<p>Using Flexbox is the easiest way! Just apply these three properties to the parent container:</p><pre><code>.parent-container {\n  display: flex;\n  justify-content: center; /* Horizontal centering */\n  align-items: center;    /* Vertical centering */\n}</code></pre>', createdAt: new Date(Date.now() - 20 * 60 * 60 * 1000).toISOString() },
            { id: 202, questionId: 101, author: 'bob', votes: 12, content: '<p>For most apps, React\'s built-in <strong>Context API</strong> is sufficient and easier to set up than Redux. You can create a context for your shared state and use a provider to wrap your component tree. Use Redux only when you have very complex state logic or need advanced middleware and developer tools.</p>', createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
            { id: 203, questionId: 102, author: 'charlie', votes: 25, content: '<ul><li>Use <code>const</code> by default.</li><li>Use <code>let</code> only when you know you need to reassign the variable.</li><li>Avoid using <code>var</code> in modern JavaScript due to its function-scoping and hoisting behavior, which can cause unexpected bugs.</li></ul>', createdAt: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000).toISOString() }
        ];
    ...
    themeToggleBtn.className = 'p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors';
    ...
    const saveDb = () => Object.keys(db).forEach(key => {
        if (key !== 'currentUser') localStorage.setItem(`stackit_${key}`, JSON.stringify(db[key]));
    });
    ...
    const pageTemplates = {
        ...
        detail: (args) => {
            ...
            <div class="p-4 sm:p-8 rounded-xl border" style="background-color: var(--bg-secondary); border-color: var(--border-color);">
                ...
                <h2 class="text-xl sm:text-2xl font-bold mt-8 mb-4">${ans.length} Answers</h2>
                <div id="answer-list" class="space-y-6">${ans.map(a => answerTemplate(a, q)).join('') || '<p>No answers yet.</p>'}</div>
            </div>
    ...
    // =================================================================================
    const pages = { home: document.getElementById('page-home'), ask: document.getElementById('page-ask'), detail: document.getElementById('page-detail'), login: document.getElementById('page-login'), signup: document.getElementById('page-signup') };
    ...
    alertModal.addEventListener('click', () => alertModal.classList.remove('active'));
    ...
    const renderQuestions = () => {
        ...
        if (paginatedQuestions.length === 0) {
            ...
            paginatedQuestions.forEach((q, index) => {
                ...
                card.className = 'p-4 sm:p-6 rounded-xl border transition-all hover:shadow-lg hover:-translate-y-1 question-card-enter';
                card.style.backgroundColor = 'var(--bg-secondary)';
                card.style.borderColor = 'var(--border-color)';
                card.style.animationDelay = `${index * 50}ms`;
                card.dataset.questionId = q.id;

                const questionVote = db.currentUser ? db.questionVotes.find(v => v.userId === db.currentUser.id && v.questionId === q.id) : null;

                card.innerHTML = `
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0 flex flex-col items-center w-12 text-center" style="color: var(--text-secondary);">
                            <button class="home-vote-btn vote-btn p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-800 ${questionVote?.type === 'up' ? 'voted-up' : ''}" data-vote="up" data-question-id="${q.id}">
                                <span class="text-xl">▲</span>
                            </button>
                            <span class="font-bold text-base sm:text-lg" style="color: var(--text-primary);">${q.votes || 0}</span>
                            <button class="home-vote-btn vote-btn p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-800 ${questionVote?.type === 'down' ? 'voted-down' : ''}" data-vote="down" data-question-id="${q.id}">
                                <span class="text-xl">▼</span>
                            </button>
                        </div>
                        <div class="flex-1 cursor-pointer">
                            <h3 class="text-base sm:text-lg font-semibold mb-2 hover:text-orange-500">${q.title}</h3>
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                                <div class="text-xs sm:text-sm" style="color: var(--text-secondary);">
                                    by <strong>${q.author}</strong> • ${formatTimeAgo(q.createdAt)}
                                </div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    ${q.tags.map(t => `<span class="tag-item px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-xs font-medium">${t}</span>`).join('')}
                                </div>
                            </div>
                        </div>
                    </div>`;
        ...
        if (totalPages > 0) {
            paginationControls.innerHTML = `
                <button data-page-nav="prev" class="px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed" style="background-color: var(--bg-secondary); border: 1px solid var(--border-color);" ${currentPage === 1 ? 'disabled' : ''}>
                    Previous
                </button>
                <span class="px-4 py-2 font-semibold">
                    Page ${currentPage} of ${totalPages}
                </span>
                <button data-page-nav="next" class="px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed" style="background-color: var(--bg-secondary); border: 1px solid var(--border-color);" ${currentPage === totalPages ? 'disabled' : ''}>
                    Next
                </button>
            `;
        }
    ...
    const initHomePage = () => {
        ...
        listEl.addEventListener('click', e => {
            ...
            if (voteBtn) {
                ...
                const existingVote = db.questionVotes.find(v => v.userId === db.currentUser.id && v.questionId === questionId);

                if (existingVote) {
                    questionToVote.votes -= (existingVote.type === 'up' ? 1 : -1);
                    db.questionVotes = db.questionVotes.filter(v => !(v.userId === db.currentUser.id && v.questionId === questionId));
                }

                if (!existingVote || existingVote.type !== voteType) {
                    questionToVote.votes += (voteType === 'up' ? 1 : -1);
                    db.questionVotes.push({ id: Date.now(), userId: db.currentUser.id, questionId, type: voteType });
                }
    ...
    const initLoginPage = () => {
        document.getElementById('login-form').addEventListener('submit', e => {
            ...
            const user = db.users.find(
                u => u.contact.toLowerCase() === contact.toLowerCase() && u.password === password
            );
            ...
            if (user) {
                ...
                showAlert('Invalid credentials. Please check your email/phone and password.');
            }
        });
    };
    ...
    const checkPasswordStrength = (passwordInput, strengthBar, strengthText) => {
        ...
        let text = 'Weak';
        if (score === 1) { text = 'Weak (needs number & special char)'; }
        if (score === 2) { color = 'bg-yellow-500'; text = 'Medium (needs one more type)'; }
        if (score === 3) { color = 'bg-green-500'; text = 'Strong'; }
        if (pass.length < 8) { text = 'Weak (at least 8 characters)'; score = 0; }
        if (pass.length === 0) { text = ''; score = 0; }
    ...
    const initSignupPage = () => {
        ...
        passwordInput.addEventListener('input', () => checkPasswordStrength(passwordInput, strengthBar, strengthText));
        form.addEventListener('submit', e => {
            e.preventDefault();
            if (checkPasswordStrength(passwordInput, strengthBar, strengthText) < 3) {
                showAlert('Please use a stronger password (at least 8 characters, one number, one special character).');
                return;
            }
            ...
            if (db.users.some(u => u.contact.toLowerCase() === contact.toLowerCase())) {
    ...
    const initAskPage = () => {
        const quill = new Quill('#editor-container', { modules: { toolbar: [[{ 'header': [1, 2, 3, false] }], ['bold', 'italic', 'strike'], ['link', 'image'], [{ 'list': 'ordered'}, { 'list': 'bullet' }], [{ 'align': [] }], ['clean']] }, theme: 'snow', placeholder: 'Write your question details here...' });
        ...
        const renderTags = () => {
            ...
            for (const tag of currentTags) {
                ...
                tagEl.className = 'tag-item p-1 px-2.5 rounded-full text-sm font-medium';
        ...
        tagsInput.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); const newTag = tagsInput.value.trim(); if (newTag && !currentTags.includes(newTag) && currentTags.length < 5) { currentTags.push(newTag); renderTags(); } tagsInput.value = ''; } });
        tagsContainer.addEventListener('click', e => { if (e.target.tagName === 'BUTTON' && e.target.dataset.tagName) { currentTags = currentTags.filter(t => t !== e.target.dataset.tagName); renderTags(); } });
        ...
        document.getElementById('ask-question-form').addEventListener('submit', e => {
            ...
            if (quill.getLength() < 15) { showAlert('Please provide more details.'); return; }
            if (currentTags.length === 0) { showAlert('Please add at least one tag.'); return; }
            db.questions.push({ id: Date.now(), title, description: descriptionHTML, tags: currentTags, author: db.currentUser.username, votes: 0, createdAt: new Date().toISOString() });
            saveDb();
            showPage('home');
        });
    };
    ...
    const initDetailPage = (question, answers) => {
        ...
        if (db.currentUser) {
            ...
            const answerQuill = new Quill('#answer-editor-container', { theme: 'snow', placeholder: 'Write your answer here...' });
            document.getElementById('answer-form').addEventListener('submit', e => {
                e.preventDefault();
                if (answerQuill.getLength() < 2) return;
                db.answers.push({ id: Date.now(), questionId: question.id, author: db.currentUser.username, votes: 0, content: answerQuill.root.innerHTML, createdAt: new Date().toISOString() });
                addNotification(`Someone answered your question: "${question.title.substring(0,20)}..."`, question.author, 'new_answer', question.id);
                saveDb();
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', (e) => {
                ...
                showConfirm("Are you sure you want to delete this question? This will also delete all its answers and cannot be undone.", () => {
                    ...
                    db.questionVotes = db.questionVotes.filter(v => v.questionId !== questionId);
                    db.notifications = db.notifications.filter(n => n.referenceId !== questionId);
                    saveDb();
    ...
    const initNotifications = () => {
        ...
        const bell = document.getElementById('notification-bell'), badge = document.getElementById('notification-badge'), dropdown = document.getElementById('notification-dropdown');
        const userNotifications = db.notifications.filter(n => n.targetUser === db.currentUser.username);
        
        const renderNots = () => {
            const unreadCount = userNotifications.filter(n => !n.read).length;
            badge.style.display = unreadCount > 0 ? 'block' : 'none';
            dropdown.innerHTML = userNotifications.length > 0 ? userNotifications.map(n => `
                <div class="notification-item p-2 border-b cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 ${n.read ? 'font-normal text-gray-400 dark:text-gray-500' : 'font-semibold'}" style="border-color: var(--border-color);" data-id="${n.id}" data-type="${n.type}" data-ref-id="${n.referenceId}">
                    ${n.text}
                </div>
            `).join('') : '<p class="text-sm p-2" style="color: var(--text-secondary);">No notifications.</p>';
        };
